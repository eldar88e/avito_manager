...
export default class extends Controller {
  ...
  _setClasses(el, active) {
    const activeClasses = (this.activeTabClassesValue || "").split(" ").filter(Boolean)
    const inactiveClasses = (this.inactiveTabClassesValue || "").split(" ").filter(Boolean)
    // базовый вид вкладки
    const base = ["px-3","py-2","text-sm","font-medium","-mb-px","border-b-2","focus:outline-none"]
    el.className = base.join(" ")
    el.classList.add(...(active ? activeClasses : ["border-transparent", ...inactiveClasses]))
  }
}
