class ApplicationController < ActionController::Base
  ...
  def set_store
    ...
    @store = current_user.stores.find(params.expect(:id))
  ...
  def cache_store
    @store = Rails.cache.fetch("user_#{current_user.id}_store_#{params[:store_id]}", expires_in: 6.hours) do
      current_user.stores.find(params.expect(:store_id))
    end
  end

  def set_search_ads
    @q_ads = @store.ads.includes(images_attachments: :blob).order(created_at: :desc).ransack(params[:q])
  end
  ...
  def settings
    @settings_ ||= Setting.all_cached(current_user&.id) || {}
  end
